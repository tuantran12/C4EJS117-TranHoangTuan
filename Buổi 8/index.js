...
// Câu 2
const inp = document.getElementById("inpSecond");
...
start.addEventListener('click',()=>{
    ...
    let timer = setInterval(()=>{
        ...
        if (time === -1) {
            ...
            clearInterval(timer);
            para.textContent = "Time'uppppp";
        }
        stop.addEventListener('click',()=>{
            clearInterval(timer);
            para.textContent = "Cancel";
        })
    },1000)
})
...
const display = () => {
    const randomIndex = Math.floor(Math.random() * quote.length + 1) - 1;
